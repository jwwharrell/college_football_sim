...
Passing means the fixed matchup matrix lies within the profile's declared aggregate envelopes; it does not prove realism for every team strength or matchup. The domain models players, season rosters, and manager-controlled depth charts, while aggregate `Team` ratings remain the default simulation inputs. The engine still lacks player-level effects, schemes, coaching, fatigue, injuries, weather, penalties, and tactical clock management. Public matchup modifiers provide an extension seam for these systems without coupling them to drive orchestration.
Depth charts can now opt into that seam through bounded, deterministic `foundational-v1` unit strengths. The canonical baseline supplies no depth-chart modifiers, so its checked-in results remain the aggregate-rating regression baseline.
